...
class FluidHandle:
    """ Create a handle to the FluidSynth library.

    An instance of this class can be used the same way any real library handle
    to FluidSynth can be used. It "implements" all necessary FluidSynth functions.
    ...
    def __init__(self, library_path=None):
        ...
        If a path is given it tries to use this path if not it searches for the library.
        """
        ...
        self.fluid_sequencer_register_client.argtypes = (c_void_p, c_char_p,
    ...
    def load_library(self, library_path):
        """ Create new FluidSynth handle with given library path.

        If no specific path is given or the file doesn't exist this class will try to find
        the library based on some basic heuristics. """

        # Search library in local context.
        if not self._is_file(library_path):
