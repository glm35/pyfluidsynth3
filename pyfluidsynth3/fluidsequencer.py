...
class FluidSequencer(dict):
    """ Represent the FluidSynth sequencer object as defined in seq.h. A instance of this class
    ...
    def __init__(self, handle, *synths):
        """ Create a new FluidSynth sequencer instance with the given handle. If not empty all
    ...
    def __del__(self):
        """ Delete the sequencer instance. """
        self.handle.delete_fluid_sequencer(self.seq)

    def __delitem__(self, key):
        """ Cancel the registration of a FluidSynth object to this sequencer. """
        id_, name = self[key]
        self.handle.fluid_sequencer_unregister_client(self.seq, id_)
    ...
    def ticks_per_beat(self):
        """ Return the current ticks per beat. """
        return self._tpb
    ...
    def add_synth(self, synth):
        ...
        id_ = self.handle.fluid_sequencer_register_fluidsynth(self.seq, synth.synth)
        name = self.handle.fluid_sequencer_get_client_name(self.seq, id_)

        self[synth] = id_, name

        return id_, name

    def is_dest(self, id_):
        """ Check if a client is a destination client. Return true if is destination client else
        false. """
        result = self.handle.fluid_sequencer_client_is_dest(self.seq, id_)
        return result == constants.TRUE

    def send(self, event, timestamp, absolute=True):
        """ Schedule an event for sending at a later time. Return true if success else false. """
